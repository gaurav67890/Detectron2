        ...
        try:
            ...
            if len(org_bbox_dict.keys())==0:
                ...
                for cl,k in zip(classes_pred,bbox_pred):
                    ...
                    if max(area_dict.values())>thresh:
                        tp_temp=tp_temp+1
                        bbox_detected.append(max(area_dict, key=area_dict.get))
                    else:
                        fp_temp=fp_temp+1
            ...
            annt_dict['True_positive']=tp_temp
            annt_dict['False_positive']=fp_temp
            annt_dict['False_negative']=fn_temp
            annt_dict['annotations_detected']=bbox_detected
...
with open(damage_name+thresh_string+'confusion_matrix.json', 'w') as outfile:
        json.dump(confusion_matrix,outfile,indent=4,ensure_ascii = False)

csv_file = thresh_string+"_data_out.csv"
csv_columns = ['image_name','True_positive','False_positive','False_negative','annotations_detected']
try:
